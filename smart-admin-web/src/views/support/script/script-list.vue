  ...
  <a-card size="small">
    ...
    <a-drawer v-model:open="detailOpen" :title="detail.scriptName || '脚本详情'" width="900" :destroy-on-close="true">
      <a-spin :spinning="detailLoading">
        ...
        <SmartCodeEditor :value="detail.content || ''" language="java" theme="vs-dark" height="360px" :readOnly="true" />

        <a-divider orientation="left">本场景可用的变量</a-divider>
        <a-table
          rowKey="name"
          :dataSource="sceneParams"
...
<script setup>
  ...
  function filterData() {
    ...
    tableData.value = allData.value.filter((item) => {
      ...
      return (
        item.scriptCode.toLowerCase().includes(keyword) || (item.scriptName || '').toLowerCase().includes(keyword)
      );
    });
  }
  ...
  const compatiblePointOptions = computed(() =>
    refPoints.value
      ...
      .map((point) => ({ value: point.refPoint, label: point.title }))
  );
