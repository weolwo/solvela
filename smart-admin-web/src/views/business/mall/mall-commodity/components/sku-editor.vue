    ...
    <div class="mb-4 pl-3 spec-section">
      ...
      <div v-for="(spec, index) in specList" :key="index" class="spec-box">
        ...
        <div class="flex items-center flex-wrap gap-2">
          ...
          <a-input
            v-model:value="spec.tempValue"
            placeholder="输入后回车"
            size="small"
            style="width: 130px"
            @press-enter="addSpecValue(index)"
          />
        </div>
      </div>
...
<script setup>
  ...
  async function generateSkuTable() {
    ...
    // 编码一次问后端要够：铁律 8 的唯一真源是 SmartCodeUtil，前端自造迟早和后端漂开
    let freshCodes = [];
    if (newCount > 0) {
  ...
  function applyBatch() {
    const targets = selectedRowKeys.value.length
      ? rows.value.filter((r) => selectedRowKeys.value.includes(r.rowKey))
      : rows.value;
    if (!targets.length) {
      return;
    }
