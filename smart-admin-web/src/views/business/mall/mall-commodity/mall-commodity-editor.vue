      ...
      <div class="editor-main">
        <a-form :model="form" layout="vertical">
          ...
          <a-card id="sec-media" :bordered="false" class="shadow-sm mb-5">
            ...
            <a-form-item label="封面主图（建议 800×800）" required :validate-status="form.coverFileId ? '' : 'error'" :help="form.coverFileId ? '' : '必填'">
            ...
            <a-form-item :label="`轮播图册（最多 ${MAX_BANNER_COUNT} 张）`">
              ...
              <div class="form-tip mt-2">
                轮播图不单独建表，复用 <code>t_file_relation(biz_type='MALL_COMMODITY_BANNER', biz_id, sort)</code> —— 那张表的 sort 列注释原文就是「附件顺序，轮播图必需」。
              </div>
...
<script setup>
  ...
  /** 铁律 8 的唯一真源在后端 SmartCodeUtil，这里是同源常量 */
  ...
  const statusMeta = computed(
    () => Object.values(COMMODITY_STATUS_ENUM).find((item) => item.value === form.status) || COMMODITY_STATUS_ENUM.DRAFT
  );
  ...
  const noticeHint = computed(() =>
    isCoupon.value ? '券类商品务必写清核销方式和有效期——这是客诉最集中的地方' : '实物建议写明发货时效与售后口径'
  );
  ...
  function buildCascader(list) {
    ...
    return roots.map((root) => {
      ...
      return children.length
        ? { value: root.id, label: root.categoryName, children }
        : { value: root.id, label: root.categoryName };
    });
  }
