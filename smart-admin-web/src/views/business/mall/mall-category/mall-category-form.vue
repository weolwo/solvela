...
<template>
  ...
  >
    ...
    <div class="batch-table">
      ...
      <template v-for="(row, index) in rows" :key="row.rowKey">
        <div class="batch-row">
          <div class="col-name">
            <a-input
              v-model:value="row.categoryName"
              placeholder="如 数码3C"
              :maxlength="50"
              :status="rowError(row, rows) ? 'error' : ''"
            />
          </div>
    ...
    <div class="batch-foot">
      ...
      <span v-else-if="editChildCount > 0" class="form-tip">
        它下面有 {{ editChildCount }} 个子分类；停用后这些子分类在 C 端也会一并不可见
      </span>
    </div>
...
<script setup>
  ...
  import {
    ...
    MAX_CATEGORY_BATCH,
    ROOT_PARENT_ID,
  } from '/@/constants/business/mall/mall-category-const';
  ...
  const editChildCount = computed(() =>
    editingId.value ? allCategoryList.value.filter((c) => c.parentId === editingId.value).length : 0
  );
  ...
  const parentOptions = computed(() => {
    const roots = allCategoryList.value
      .filter((c) => !c.parentId && c.id !== editingId.value)
  ...
  const existingSiblingNames = computed(() => {
    ...
    return allCategoryList.value
      .filter((c) => (c.parentId || ROOT_PARENT_ID) === pid && c.id !== editingId.value)
