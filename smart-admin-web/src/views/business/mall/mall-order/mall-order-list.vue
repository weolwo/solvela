...
<template>
  ...
  <a-card size="small" :bordered="false" class="stat-card" :loading="statLoading">
    <div class="stat-head" @click="statOpen = !statOpen">
      ...
      <span v-if="!statOpen" class="stat-collapsed">
        {{ stat.orderCount }} 单 · {{ stat.quantitySum }} 件 · 消耗 {{ stat.payPointsSum }} 积分
      </span>
    </div>
    ...
    <div v-show="statOpen" class="stat-body">
      ...
      <a-table
        size="small"
        row-key="commodityId"
        :data-source="stat.commodityRank"
        :columns="rankColumns"
        :pagination="false"
        bordered
      >
  ...
  async function queryData() {
    ...
    try {
      ...
      const [pageRes, statRes] = await Promise.all([
        mallOrderApi.queryPage(queryForm),
        mallOrderApi.queryStat(queryForm),
      ]);
