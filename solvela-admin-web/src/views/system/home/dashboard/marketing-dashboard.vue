    ...
    <a-row :gutter="[10, 10]" class="main-row">
      ...
      <a-col :span="6" class="side-col">
        ...
        <a-card size="small" class="panel activity-panel fill-card">
          ...
          <div class="caliber">
            起止时间只做展示，不参与状态判定。完备度来自 ActivityRefProvider.checkConfigured （判据是「能不能上线」，不是「有没有记录」）。
          </div>
        </a-card>
      </a-col>
      ...
      <a-col :span="6" class="side-col">
        ...
        <a-card size="small" title="事件丢弃原因" class="panel">
          ...
          <div class="caliber">
            按 discard_code 聚类，不按自由文本的 discard_reason（后者带具体数值，是给人读的， 直接分组会炸成几百个值）。「需介入」的判据在后端
            TaskDiscardCode 枚举里。
          </div>
        </a-card>
