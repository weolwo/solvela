...
<template>
  ...
  <div class="funnel-toggle">
    ...
    <span v-if="!funnelOpen" class="funnel-summary">
      ...
      <span :class="Number(funnel.noStockRate ?? 0) >= 0.2 ? 'summary-alert' : ''"> 库存不足 {{ percent(funnel.noStockRate) }} </span>
    </span>
  </div>
    ...
    <a-row :gutter="12" class="overview-row">
      <a-col :span="5">
        <div class="overview-card tone-primary">
          ...
          <div class="overview-card-foot">{{ num(funnel.memberCount) }} 人参与 · 人均 {{ funnel.drawPerMember ?? '-' }} 次</div>
        </div>
      </a-col>
      <a-col :span="5">
    ...
    <a-card v-if="funnel.prizeHitList && funnel.prizeHitList.length > 0" size="small" :bordered="false" class="prize-card">
      ...
      <div class="prize-list">
        <div v-for="item in funnel.prizeHitList" :key="item.prizeCode" class="prize-row">
          <div class="prize-name">{{ prizeIcon(item.prizeType) }} {{ item.prizeName || item.prizeCode }}</div>
  ...
  <a-form class="solvela-query-form">
    <a-row class="solvela-query-form-row">
      ...
      <a-form-item label="请求ID" class="solvela-query-form-item">
