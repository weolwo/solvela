...
<template>
  ...
  <a-form class="solvela-query-form">
    <a-row class="solvela-query-form-row">
      <a-form-item label="彩票编码" class="solvela-query-form-item">
        <a-input style="width: 200px" v-model:value="queryForm.lotteryCode" placeholder="彩票编码" />
      </a-form-item>
      <a-form-item label="彩票名称" class="solvela-query-form-item">
      ...
      <template #bodyCell="{ text, record, column }">
        ...
        <template v-if="column.dataIndex === 'spaceUsage'">
          <div class="cell-stack">
            ...
            <span class="cell-sub"> {{ record.numberLength }} 位 · 上限 {{ num(record.totalCount) }} / 空间 {{ num(record.numberSpace) }} </span>
          </div>
        </template>
