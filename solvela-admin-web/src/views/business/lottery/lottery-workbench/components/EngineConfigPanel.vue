        ...
        <a-col :span="12">
          <a-form layout="vertical">
              ...
              <a-col :span="12">
                <a-form-item required class="mb-3">
                  ...
                  <a-input-group compact>
                    <a-input v-model:value="form.lotteryCode" style="width: calc(100% - 76px)" :disabled="configured" placeholder="如 H88JHKJFNE" />
            ...
            <div v-else class="text-slate-400 text-xs mt-2">
              发售 {{ (form.totalCount || 0).toLocaleString() }} / 空间 {{ domain.toLocaleString() }}， 占用
              {{ occupancy }}%。空间越宽裕，号码越稀疏、越难被猜中。
            </div>
          </a-form>
        </a-col>
      ...
      <div class="w-7/12 flex flex-col gap-3">
        <div class="flex justify-between items-center mb-1">
          ...
          <div class="flex gap-2">
            ...
            <a-button type="primary" size="small" class="bg-green-600 border-green-600 font-bold" @click="addRule"> ➕ 新增奖级 (置顶) </a-button>
          </div>
        </div>
        ...
        <div class="bg-slate-50 p-3 rounded-lg border border-slate-200 max-h-[600px] overflow-y-auto flex flex-col gap-3 shadow-inner">
          <div v-if="form.prizeRuleList.length === 0" class="py-8 text-center text-slate-400 text-sm">暂无奖级配置，请点击上方新增</div>
          <div
          ...
          >
            ...
            <div class="flex items-center gap-2 overflow-hidden">
              ...
              <div class="truncate leading-tight">
                ...
                <div class="text-[11px] text-slate-400 font-mono mt-0.5">{{ asset.prizeCode }} · 价值 {{ asset.prizeValue }}</div>
              </div>
            </div>
          </div>
        </div>
