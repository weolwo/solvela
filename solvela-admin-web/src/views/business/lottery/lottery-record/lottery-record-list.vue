...
<template>
  ...
  <div class="funnel-toggle">
    ...
    <span v-if="!funnelOpen" class="funnel-summary">
      ...
      <span :class="funnel.dispatchFailedCount > 0 ? 'summary-alert' : ''"> 派发失败 {{ num(funnel.dispatchFailedCount) }} </span>
    </span>
  </div>
  ...
  <div v-show="funnelOpen">
    <a-row :gutter="12" class="overview-row">
      <a-col :span="6">
        <div class="overview-card tone-primary">
          ...
          <div class="overview-card-foot">{{ num(funnel.memberCount) }} 人参与 · 人均 {{ funnel.ticketPerMember ?? '-' }} 张</div>
        </div>
      </a-col>
      <a-col :span="6">
      ...
      <a-col :span="6">
        ...
        <div class="overview-card" :class="funnel.dispatchFailedCount > 0 ? 'tone-critical' : 'tone-muted'">
          ...
          <div class="overview-card-foot">已投递 {{ num(funnel.dispatchedCount) }} · 待派发 {{ num(funnel.dispatchWaitCount) }}</div>
        </div>
      </a-col>
    </a-row>
    ...
    <a-alert v-for="(issue, idx) in funnel.issueList || []" :key="idx" type="error" show-icon class="mb-2">
      <template #message
        ><span class="text-xs">{{ issue }}</span></template
      >
    </a-alert>
  ...
  <a-form class="solvela-query-form">
    <a-row class="solvela-query-form-row">
      ...
      <a-form-item label="期号" class="solvela-query-form-item">
        <a-select
          style="width: 180px"
      ...
      <a-form-item label="号码" class="solvela-query-form-item">
        <a-input style="width: 150px" v-model:value="queryForm.ticketNumber" placeholder="精确查号" @pressEnter="onSearch" />
      </a-form-item>
      <a-form-item label="会员名" class="solvela-query-form-item">
        <a-input-number
          style="width: 150px"
          v-model:value="queryForm.memberId"
          placeholder="会员号"
          :controls="false"
          :precision="0"
          @pressEnter="onSearch"
      ...
      <a-form-item label="中奖状态" class="solvela-query-form-item">
        <a-select
          style="width: 130px"
          v-model:value="queryForm.winStatus"
          :options="WIN_STATUS_OPTIONS"
          placeholder="全部"
          allowClear
          @change="onSearch"
        />
      </a-form-item>
      <a-form-item label="领号时间" class="solvela-query-form-item">
