...
<template>
  ...
  <a-form class="solvela-query-form">
    <a-row class="solvela-query-form-row">
      ...
      <a-form-item label="奖品编码" class="solvela-query-form-item">
        <a-input style="width: 160px" v-model:value="queryForm.prizeCode" placeholder="奖品编码" allow-clear />
      </a-form-item>
      <a-form-item label="审批状态" class="solvela-query-form-item">
        <a-select style="width: 140px" v-model:value="queryForm.approveStatus" :options="APPROVE_STATUS_OPTIONS" placeholder="全部" allow-clear />
      </a-form-item>
      <a-form-item label="执行状态" class="solvela-query-form-item">
        <a-select style="width: 140px" v-model:value="queryForm.status" :options="DISPATCH_STATUS_OPTIONS" placeholder="全部" allow-clear />
      </a-form-item>
      <a-form-item label="发放时间" class="solvela-query-form-item">
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.dataIndex === 'approveStatus'">
          ...
          <div v-if="record.approveBy" class="text-[11px] text-slate-400 mt-0.5">{{ record.approveBy }} · {{ fmt(record.approveTime) }}</div>
        </template>
    ...
    <a-modal v-model:open="rejectVisible" title="驳回发放" ok-text="确认驳回" cancel-text="取消" @ok="doReject">
      <div v-if="rejectTarget" class="text-sm mb-3">
        将驳回给 <b>{{ rejectTarget.memberName }}</b> 的 <b>{{ rejectTarget.prizeName }}</b
        >（{{ rejectTarget.prizeType }} / {{ rejectTarget.prizeValue }}）
      </div>
