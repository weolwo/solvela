  ...
  >
    <a-alert type="warning" show-icon class="mb-4">
      <template #message>
        <span class="text-xs leading-relaxed">
          补发会新建一条<b>待审批</b>的发奖记录，此刻还<b>不会</b>发出任何东西。 回到列表点「通过」才会真正走派发链路（生成提案 → 扣减优惠配置预算 →
          账务到账）。
        </span>
      </template>
    </a-alert>
    ...
    <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 5 }" :wrapper-col="{ span: 17 }">
      ...
      <a-form-item label="过期时间" name="validUntil">
        <a-date-picker v-model:value="form.validUntil" show-time valueFormat="YYYY-MM-DD HH:mm:ss" class="w-full" placeholder="不填表示不过期" />
      </a-form-item>
      ...
      <a-form-item label="幂等单号">
        ...
        <div class="mt-1 text-xs text-slate-400">自动生成并落在唯一索引上，重复提交同一单号会被库直接挡下，避免手抖补两次</div>
      </a-form-item>
    </a-form>
