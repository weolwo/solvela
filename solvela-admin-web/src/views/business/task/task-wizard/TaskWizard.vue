...
<template>
  <a-card :bordered="false">
    ...
    <a-result v-if="submitResult.submitted && !embedded" status="success" :title="isEditMode ? '任务配置更新成功' : '任务配置提交成功'">
      <template #subTitle>
    ...
    <template v-else>
      ...
      <a-alert v-if="pendingDraft" type="warning" show-icon class="mb-4">
        <template #message>
          发现上次未提交的配置草稿（暂存于 {{ pendingDraft.savedAt }}，任务名称 {{ pendingDraft.wizardForm?.base?.taskName || '未填写' }}）
        </template>
        <template #action>
          <a-space>
      ...
      <a-alert v-if="isEditMode" type="info" show-icon class="mb-4">
        <template #message>
          正在编辑任务「{{ wizardForm.base.taskName || '—' }}」（ID {{ editId }}）。 保存后覆盖原配置，奖励阶梯整体替换；<b>归属活动不可更改</b>。
      ...
      <a-form ref="formRef" :model="wizardForm" layout="vertical">
        ...
        <div v-show="currentStep === WIZARD_STEP.BASE">
          <a-card size="small" title="选择任务模板" class="mb-4" :loading="templateLoading">
            <a-empty v-if="!templateLoading && taskTemplates.length === 0" description="还没有任何任务模板，请先到「任务模板设计器」配置一个" />
            <a-row :gutter="[12, 12]">
        ...
        <div v-show="currentStep === WIZARD_STEP.RULE">
          <a-card size="small">
            ...
            <SchemaFormRenderer v-model="wizardForm.ruleParams" :schema-params="schemaParams" :name-prefix="RULE_PARAMS_NAME_PREFIX" />
          ...
          -->
            <a-form-item v-if="isRoundLimitSupported" label="周期内可完成轮数 limit_count" :name="['limit', 'limitCount']">
              <a-input-number v-model:value="wizardForm.limit.limitCount" :min="1" />
              <div class="text-xs text-gray-400! mt-1">一个周期内最多可完成几轮任务、领几次奖。完成一轮后自动开下一轮，用满为止</div>
            </a-form-item>
          </a-card>
        </div>
        ...
        <div v-show="currentStep === WIZARD_STEP.AUDIENCE">
          ...
          <a-card size="small" title="C端展示与规则说明">
            <a-form-item label="任务副标题 task_desc" :name="['display', 'taskDesc']" extra="展示在 C 端任务名称下方，一句话吸引用户">
            ...
            >
              <SolvelaRichEditor v-model="wizardForm.display.ruleDesc" />
            </a-form-item>
          </a-card>
        </div>
...
<script setup>
  ...
  function validatePrizeLadders() {
    const missing = wizardForm.prizeLadders.map((row, index) => (row.prizeCode ? null : index + 1)).filter((i) => i !== null);
    if (missing.length) {
