...
<template>
      ...
      <a-col :span="6">
        <div class="overview-card tone-success">
          <div class="overview-card-label">
            达标率
            <a-tooltip
              title="分母是接取总数。任务不像开奖有明确的揭晓时刻，「进行中」本身就是一种结果（做不完也是做不完），把它剔出分母会让达标率虚高"
            >
      ...
      <a-col :span="6">
        ...
        <div class="overview-card" :class="funnel.staleRunningCount > 0 ? 'tone-critical' : 'tone-muted'">
          <div class="overview-card-label">
            过期未收口
            <a-tooltip
              title="已过有效期却仍是「进行中」。工程里没有过期扫描任务，它们不会自己变成已过期，用户端会一直看到一个永远完不成的任务。可勾选后用「批量禁用」置为已过期"
            >
              <QuestionCircleOutlined class="overview-card-hint" />
            </a-tooltip>
          </div>
        ...
        <a-card v-if="funnel.discardList && funnel.discardList.length > 0" size="small" :bordered="false" class="stat-card">
          ...
          <div v-for="item in funnel.discardList" :key="item.discardCode || 'UNKNOWN'" class="stat-row">
            ...
            <div class="stat-bar-wrap">
              <div
                class="stat-bar"
                :class="item.needsAttention ? 'stat-bar-alert' : ''"
                :style="{ width: barPercent(item.discardShare) + '%' }"
              ></div>
            </div>
  ...
  <a-form class="solvela-query-form">
    <a-row class="solvela-query-form-row">
      ...
      <a-form-item label="达标时间" class="solvela-query-form-item">
  ...
  <a-drawer :open="flowVisible" title="任务事件流水" :width="900" @close="flowVisible = false">
    <a-alert type="info" show-icon class="mb-3">
      <template #message>
        这里是这条任务记录收到过的<b>全部事件</b>，含<b>被丢弃的</b>及其原因。 「用户下了 99 元的单为什么没进度」这类问题，答案就在丢弃原因里。
      </template>
    </a-alert>
