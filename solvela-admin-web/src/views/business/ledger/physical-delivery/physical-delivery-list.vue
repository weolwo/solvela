...
<template>
  ...
  <StatPanel title="发货统计" storage-key="physical-delivery" :issue-list="stat.issueList" @change="loadStat">
    <template #summary>
      ...
      <span :class="stat.pendingCount > 0 ? 'solvela-stat-summary-alert' : ''"> 待发货 {{ num(stat.pendingCount) }} 单 </span>
    ...
    <a-row :gutter="12" class="solvela-stat-row">
      <a-col :span="5">
        <div class="solvela-stat-card is-primary">
          <div class="solvela-stat-card-label">本期新增</div>
          <div class="solvela-stat-card-value">
            {{ num(stat.newCount) }}
            <span class="solvela-stat-card-unit">单</span>
          </div>
          <div class="solvela-stat-card-foot">涉及 {{ num(stat.newMemberCount) }} 名会员</div>
        </div>
      </a-col>
      <a-col :span="5">
        <!-- 积压是存量：限制在今天，压了三天的单子会正好从页面上消失 -->
        <div class="solvela-stat-card" :class="pendingTone">
          ...
          <div class="solvela-stat-card-foot">最久等待 {{ pendingWaitedText }}</div>
        </div>
      </a-col>
      <a-col :span="5">
        <!-- 这才是运营今天真正能干的活 -->
        <div class="solvela-stat-card" :class="stat.pendingReadyCount > 0 ? 'is-warning' : 'is-muted'">
          <div class="solvela-stat-card-label">
            可直接发
            <span class="solvela-stat-card-tag">全量</span>
          ...
          <div class="solvela-stat-card-foot">收件信息齐全，可安排出库</div>
        </div>
      </a-col>
      <a-col :span="5">
        ...
        <div class="solvela-stat-card is-muted">
          <div class="solvela-stat-card-label">
            ...
            <span class="solvela-stat-card-tag">全量</span>
            <a-tooltip
              title="待发货里收件人/电话/地址任一为空的单子。实物是三段式履约，中奖时用户还没填地址——这批单子想发也发不了，要去催用户，不是仓库的活"
            >
              <QuestionCircleOutlined class="solvela-stat-card-hint" />
            </a-tooltip>
          </div>
          ...
          <div class="solvela-stat-card-foot">催用户，不是催仓库</div>
        </div>
      </a-col>
      <a-col :span="4">
        <div class="solvela-stat-card" :class="stat.returnedCount > 0 ? 'is-critical' : 'is-success'">
          <div class="solvela-stat-card-label">
            发货率
      ...
      <a-col :span="12">
        <a-card v-if="stat.sourceList && stat.sourceList.length > 0" size="small" :bordered="false" class="solvela-stat-block">
          <div class="solvela-stat-block-head">
            来源分布（条形为发货率）
            <span class="solvela-stat-block-sub">全量，取值原样回显（这一列没有枚举约束）</span>
          </div>
          <div v-for="item in stat.sourceList" :key="item.sourceType || 'none'" class="solvela-stat-line">
            ...
            <div class="solvela-stat-bar-wrap">
              <div class="solvela-stat-bar" :style="{ width: barPercent(item.deliveredRate) + '%' }"></div>
            </div>
            <div class="solvela-stat-num">
              {{ num(item.deliveryCount) }} 单 · {{ percent(item.deliveredRate) }}
              <span v-if="item.pendingCount > 0" class="solvela-stat-alert-text"> · 待发 {{ num(item.pendingCount) }} </span>
            </div>
          </div>
        </a-card>
      </a-col>
  ...
  <a-form class="solvela-query-form">
    <a-row class="solvela-query-form-row">
      ...
      <a-form-item label="来源类型" class="solvela-query-form-item">
        <a-input style="width: 200px" v-model:value="queryForm.sourceType" placeholder="来源类型" />
      </a-form-item>
      <a-form-item label="物流单号" class="solvela-query-form-item">
...
<script setup>
  ...
  import { DELIVERY_STATUS_ENUM, DELIVERY_STATUS_OPTIONS, deliveryStatusOf } from '/src/constants/business/ledger/physical-delivery-const';
