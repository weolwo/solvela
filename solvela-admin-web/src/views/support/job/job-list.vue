        ...
        <a-tab-pane key="1" tab="有效任务">
          <a-form class="solvela-query-form">
            <a-row class="solvela-query-form-row">
              ...
              <a-form-item label="触发类型" class="solvela-query-form-item">
                <a-select style="width: 155px" v-model:value="queryForm.triggerType" placeholder="请选择触发类型" allowClear>
                  <a-select-option v-for="item in $solvelaEnumPlugin.getValueDescList('TRIGGER_TYPE_ENUM')" :key="item.value" :value="item.value">
                    {{ item.desc }}
                  </a-select-option>
                </a-select>
              </a-form-item>
              ...
              <a-form-item class="solvela-query-form-item solvela-margin-left10">
                ...
                <a-button class="solvela-margin-left20" type="primary" @click="openUpdateModal()" v-privilege="'support:job:add'">
                  <template #icon>
                    <plus-outlined />
                  </template>
                  添加任务
          ...
          <a-flex align="end" vertical>
            <div class="solvela-table-setting-block solvela-margin-bottom10">
              <TableOperator
                class="solvela-margin-bottom5 pull-right"
                v-model="columns"
