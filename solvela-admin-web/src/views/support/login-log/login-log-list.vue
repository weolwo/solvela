...
-->
<template>
  <a-form class="solvela-query-form" v-privilege="'support:loginLog:query'" ref="queryFormRef">
    <a-row class="solvela-query-form-row">
      ...
      <a-form-item label="用户IP" class="solvela-query-form-item">
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    <a-row justify="end" ref="tableOperatorRef">
      <TableOperator class="solvela-margin-bottom5" v-model="columns" :tableId="TABLE_ID_CONST.SUPPORT.LOGIN_LOG" :refresh="ajaxQuery" />
    </a-row>
    <a-table
      size="small"
    ...
    >
      <template #bodyCell="{ text, record, column }">
        ...
        <template v-if="column.dataIndex === 'userType'">
          <span>{{ $solvelaEnumPlugin.getDescByValue('USER_TYPE_ENUM', text) }}</span>
        </template>
      </template>
    </a-table>
