...
<template>
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    <a-row justify="end">
      <TableOperator class="solvela-margin-bottom5" v-model="columns" :tableId="TABLE_ID_CONST.SUPPORT.CONFIG" :refresh="ajaxQuery" />
    </a-row>
    <a-table size="small" :loading="tableLoading" :dataSource="tableData" :columns="columns" bordered rowKey="operateLogId" :pagination="false">
      <template #bodyCell="{ text, record, column }">
        ...
        <template v-if="column.dataIndex === 'operateUserType'">
          <div>{{ $solvelaEnumPlugin.getDescByValue('USER_TYPE_ENUM', text) }}</div>
        </template>
