...
<template>
    ...
    <a-form class="solvela-query-form">
      <a-row class="solvela-query-form-row">
        ...
        <a-form-item class="solvela-query-form-item">
          ...
          <a-button @click="resetQuery" class="solvela-margin-left10">重置</a-button>
        </a-form-item>
      </a-row>
    </a-form>
  ...
  <a-modal
    v-model:open="categoryModalFlag"
    :title="categoryForm.categoryId ? '编辑分类' : '新建分类'"
    @ok="saveCategory"
    :confirm-loading="categorySaving"
  >
    <a-form :label-col="{ span: 5 }" :wrapper-col="{ span: 17 }" class="mt-4">
      <a-form-item label="分类编码" required>
        <a-input v-model:value="categoryForm.categoryCode" placeholder="字母数字下划线短横线" :disabled="categoryForm.systemFlag" :maxlength="50" />
