  ...
  <a-modal v-model:open="visible" :width="900" title="选择人员" @cancel="closeModal" @ok="onSelectEmployee">
    <a-form class="solvela-query-form">
      <a-row class="solvela-query-form-row">
        ...
        <a-form-item label="部门" class="solvela-query-form-item">
        ...
        <a-form-item class="solvela-query-form-item solvela-margin-left10">
          ...
          <a-button @click="reset" class="solvela-margin-left10">
            <template #icon>
              <ReloadOutlined />
            </template>
            重置
    ...
    >
      <template #bodyCell="{ text, column }">
        ...
        <template v-if="column.dataIndex === 'gender'">
          <span>{{ $solvelaEnumPlugin.getDescByValue('GENDER_ENUM', text) }}</span>
        </template>
      </template>
    </a-table>
