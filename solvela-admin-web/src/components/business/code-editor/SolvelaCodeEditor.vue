...
-->
<template>
  <div ref="containerRef" class="solvela-code-editor" :style="{ height: normalizedHeight }"></div>
</template>
...
<script setup>
  ...
  const hasDictionary = () =>
    !!(
      props.dictionary &&
      (Object.keys(props.dictionary.objects || {}).length || props.dictionary.functions?.length || props.dictionary.keywords?.length)
    );
...
<style scoped>
  ...
  .solvela-code-editor :deep(.cm-editor) {
    height: 100%;
  }

  .solvela-code-editor :deep(.cm-scroller) {
