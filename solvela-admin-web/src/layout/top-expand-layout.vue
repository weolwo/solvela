...
<template>
  <a-layout class="admin-layout" style="min-height: 100%">
    ...
    <a-layout class="admin-layout-main" :style="`height: ${windowHeight}px`" id="solvelaAdminMain">
      <!-- 顶部头部信息 -->
      <a-layout-header class="solvela-layout-header">
        <a-row justify="space-between" class="solvela-layout-header-user">
          <a-col
            class="solvela-layout-header-left"
            :style="{
          ...
          <!---用戶操作区域-->
          <a-col class="solvela-layout-header-right">
            <HeaderUserSpace />
          </a-col>
        </a-row>
      ...
      <!--中间内容-->
      <a-layout-content class="admin-layout-content" id="solvelaAdminLayoutContent">
      ...
      <!-- footer 版权公司信息 -->
      <a-layout-footer class="solvela-layout-footer" v-show="footerFlag">
        <SolvelaFooter />
      </a-layout-footer>
      <!---- 回到顶部 --->
...
<script setup>
  ...
  import SolvelaFooter from './components/solvela-footer/index.vue';
  import { solvelaKeepAlive } from './components/solvela-keep-alive';
  import IframeIndex from '/@/components/framework/iframe/iframe-index.vue';
  import watermark from '../lib/solvela-watermark';
  ...
  function sizeComputed() {
    ...
    const parentElement = document.querySelector('.solvela-layout-header-user');
    const rightElement = document.querySelector('.solvela-layout-header-right');
  ...
  onMounted(() => {
    if (watermarkFlag.value) {
      watermark.set('solvelaAdminLayoutContent', useUserStore().actualName);
    } else {
      watermark.clear();
    }
  });
  ...
  //回到顶部
  const backTopTarget = () => {
    return document.getElementById('solvelaAdminMain');
  };
  // ----------------------- keep-alive相关 -----------------------
  let { route, keepAliveIncludes, iframeNotKeepAlivePageFlag, keepAliveIframePages } = solvelaKeepAlive();
...
<style scoped lang="less">
  ...
  .solvela-layout-header {
    background: @color-bg-container;
    padding: 0;
    z-index: 21;
  }

  .solvela-layout-header-user {
    height: @header-user-height;
    border-bottom: 1px solid @color-border-secondary;
  }

  .solvela-layout-header-left {
    display: flex;
    height: @header-user-height;
