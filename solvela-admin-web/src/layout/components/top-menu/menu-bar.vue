  ...
  >
    <template v-for="(group, index) in menuTree" :key="group.menuId">
      <template v-if="index < visibleCount">
        <!-- 有子菜单：悬浮弹出 iOS 卡片 -->
        <a-popover v-if="hasChildren(group)" placement="bottomLeft" trigger="hover" :arrow="false" :overlay-inner-style="OVERLAY_INNER_STYLE">
          <template #content>
            <div
    ...
    <!-- 放不下的收进这里 -->
    <a-popover v-if="overflowGroups.length" placement="bottomRight" trigger="hover" :arrow="false" :overlay-inner-style="OVERLAY_INNER_STYLE">
      <template #content>
        <div
...
<script setup>
  ...
  provide('solvelaSideMenu', { isOpen, isSelected, onToggle, onNavigate });
  ...
  /**
   * Solvela中 router的name 就是 后端存储menu的id
