...
<template>
  ...
  <a-row style="position: relative" v-show="pageTagFlag">
      ...
      <div class="solvela-page-tag">
        <a-tabs style="width: 100%" :tab-position="mode" v-model:activeKey="selectedKey" size="small" @tabClick="selectTab">
          <a-tab-pane v-for="item in tagNav" :key="item.menuName">
            <template #tab>
              <span class="solvela-page-tag-span">
                <home-outlined v-if="item.menuName === HOME_PAGE_NAME" class="solvela-page-tag-close solvela-page-tag-icon" />
                <component class="solvela-page-tag-icon" v-else :is="$antIcons[item.menuIcon]" />
                {{ item.menuTitle }}
                <close-outlined @click.stop="closeTag(item, false)" v-if="item.menuName !== HOME_PAGE_NAME" class="solvela-page-tag-close" />
              </span>
            </template>
          </a-tab-pane>
        </a-tabs>
    ...
    <a-dropdown>
      <!--标签页操作区-->
      <div class="solvela-page-tag-operate">
        <div class="solvela-page-tag-operate-icon">
          <AppstoreOutlined />
        </div>
      </div>
      <template #overlay>
...
<style scoped lang="less">
  @solvela-page-tag-operate-width: 40px;
  @color-primary: v-bind('token.colorPrimary');
  @color-primary-bg: v-bind('token.colorPrimaryBg');

  .solvela-page-tag-operate {
    width: @solvela-page-tag-operate-width;
    height: @solvela-page-tag-operate-width;
    font-size: 17px;
    text-align: center;
    vertical-align: middle;
    line-height: @solvela-page-tag-operate-width;
    padding-right: 10px;
    cursor: pointer;

    .solvela-page-tag-operate-icon {
      width: 20px;
      height: 20px;
      transition: all 1s;
    ...
    .solvela-page-tag-operate-icon:hover {
      width: 20px;
      height: 20px;
  ...
  .solvela-page-tag-operate:hover {
    color: @color-primary;
  }

  .solvela-page-tag-span:first-child {
    padding-right: 15px;

    .solvela-page-tag-icon {
      font-size: 12px;
    }
  }
  ...
  .solvela-page-tag {
    ...
    padding-left: 20px;
    user-select: none;
    width: calc(100% - @solvela-page-tag-operate-width);

    .solvela-page-tag-icon {
      margin-right: 4px;
    }

    .solvela-page-tag-close {
      margin-left: 5px;
      font-size: 10px;
      color: #666666;
    }
