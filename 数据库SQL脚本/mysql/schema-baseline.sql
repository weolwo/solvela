...
CREATE TABLE `t_member_grade` (
  ...
  `points_discount` tinyint unsigned DEFAULT NULL COMMENT '积分折扣率 1-100，如 90=9折；NULL 或 100 表示不打折',
...
CREATE TABLE `t_member_coupon` (
  ...
  `update_by` varchar(64) DEFAULT NULL COMMENT '更新人',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `template_version` int DEFAULT NULL COMMENT '发券时的模板版本，排查用。NULL=发券时没有模板',
  `discount_type` varchar(16) DEFAULT NULL COMMENT '规则快照：FIXED-固定金额/PERCENT-百分比。NULL=这张券没有规则',
  `discount_value` decimal(10,2) DEFAULT NULL COMMENT '规则快照：抵扣额或折扣率。NULL=没有规则，不是减 0',
  `min_amount` decimal(10,2) DEFAULT NULL COMMENT '规则快照：最低消费门槛。0=无门槛，NULL=没有规则',
  `max_discount` decimal(10,2) DEFAULT NULL COMMENT '规则快照：最高抵扣。PERCENT 必填',
  `deduct_target` varchar(16) DEFAULT NULL COMMENT '规则快照：CASH-抵现金/SCORE-抵积分。NULL=没有规则',
  `scope_type` varchar(16) DEFAULT NULL COMMENT '规则快照：ALL/COMMODITY/CATEGORY/EXTERNAL。NULL=没有规则',
...
CREATE TABLE `t_pool_prize_mapping` (
  ...
  `probability` decimal(8,4) NOT NULL DEFAULT '0.0000' COMMENT '中奖概率，百分比：10.9500 表示 10.95%，同池之和须为 100',
...
CREATE TABLE `t_mall_commodity` (
  ...
  `grade_price_flag` tinyint NOT NULL DEFAULT '1' COMMENT '参与等级折扣：0-不参与（成本价/秒杀品）, 1-参与',
...
CREATE TABLE `t_mall_order` (
  ...
  `grade_code` int NOT NULL DEFAULT '0' COMMENT '下单时的等级快照。0=无等级或未登录',
  `grade_discount` int NOT NULL DEFAULT '0' COMMENT '等级折扣让掉的积分（合计，非单价）。0=没享受折扣',
  `pay_points` int NOT NULL DEFAULT '0' COMMENT '实付积分合计',
  `pay_cash` decimal(10,2) NOT NULL DEFAULT '0.00' COMMENT '实付现金合计',
  `coupon_id` bigint DEFAULT NULL COMMENT '用掉的会员券 id。软引用，不加外键。NULL=这一单没用券',
  ...
  `update_by` varchar(64) DEFAULT NULL COMMENT '更新人',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `coupon_discount` decimal(10,2) DEFAULT NULL COMMENT '券抵扣了多少（阶段 4 只抵积分）。冗余，权威在 t_coupon_write_off；为订单详情零 join 显示',
  PRIMARY KEY (`id`),
