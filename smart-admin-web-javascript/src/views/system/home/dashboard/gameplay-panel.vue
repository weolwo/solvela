  ...
  <a-spin :spinning="loading">
    ...
    <div v-else-if="data.activityType === 'DRAW'" class="draw-box">
      ...
      <div v-for="row in drawRows" :key="row.status" class="draw-row">
        ...
        <a-progress
          class="draw-bar"
          :percent="pct(row.count, drawTotal)"
    ...
    <div v-else-if="data.activityType === 'LOTTERY'" class="lottery-box">
      ...
      <div class="caliber">
        玩法编码 lottery_code 与活动编码 activity_code 是<b>两套编码</b>，靠 t_lottery_config.activity_code
        关联 —— 一个活动可以挂多个彩票玩法，别复用同一个串。
      </div>
    </div>
