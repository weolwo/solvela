  ...
  <div>
    ...
    <a-card :bordered="false" class="shadow-sm">
        ...
        <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
          ...
          <a-form-item label="玩法类型" name="activityType">
            ...
            <div class="text-xs mt-2" :class="isBasic ? 'text-green-600' : 'text-slate-400'">
              ...
              <template v-else>
                创建后类型即冻结（已配的玩法配置会因改类型变成孤儿数据）。不确定就先建<b>基础活动</b>，之后再升级。
              </template>
            </div>
          </a-form-item>
        </a-form>
      ...
      <div v-else-if="step === STEP.GAMEPLAY" class="p-6">
        ...
        <component
          :is="gameplayComponent"
          v-if="gameplayComponent"
          :activity-code="created.activityCode"
          :embedded="true"
          @saved="onGameplaySaved"
        />
      ...
      <div v-else class="p-6">
        ...
        >
          ...
          <div v-if="isBasic" class="text-center text-xs text-slate-500">
            基础活动尚未挂载任何玩法。之后想加抽奖/任务/彩票，可在活动列表点「升级玩法」——
            它没有玩法下游，升级不会产生孤儿数据。
          </div>
        </a-result>
      </div>
