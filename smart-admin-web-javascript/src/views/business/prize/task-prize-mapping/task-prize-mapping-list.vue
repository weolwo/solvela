...
<template>
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    <TaskPrizeMappingForm ref="formRef" @reloadList="queryData" />
  </a-card>
</template>
<script setup>
  ...
  async function requestDelete(data) {
    ...
    try {
      ...
      await taskPrizeMappingApi.delete(data.id);
  ...
  async function requestBatchDelete() {
    try {
      SmartLoading.show();
      await taskPrizeMappingApi.batchDelete(selectedRowKeyList.value);
      message.success('删除成功');
      queryData();
    } catch (e) {
      smartSentry.captureError(e);
