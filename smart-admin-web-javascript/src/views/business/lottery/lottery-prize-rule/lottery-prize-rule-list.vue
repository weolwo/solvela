  ...
  <a-spin :spinning="loading">
    <a-alert type="info" show-icon class="mb-3">
      ...
      <template #description>
        <span class="text-xs">
          工作台保存奖级时按玩法整表重建，所以任何其他写入路径都会被覆盖 —— 本页刻意不提供编辑入口，避免改了个寂寞。
        </span>
      </template>
    </a-alert>
    ...
    >
      ...
      <div class="lottery-head">
        ...
        <a-button type="link" size="small" :disabled="!item.activityCode" @click="gotoWorkbench(item)">
          去工作台修改 →
        </a-button>
      </div>
          ...
          <template v-if="column.dataIndex === 'prizeCode'">
            <div class="cell-stack">
              <span v-if="record.prizeName">
                {{ prizeIcon(record.prizeType) }} {{ record.prizeName }}
              </span>
