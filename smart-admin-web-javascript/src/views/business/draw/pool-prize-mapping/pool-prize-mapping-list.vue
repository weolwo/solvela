  ...
  <a-spin :spinning="loading">
    <a-alert type="info" show-icon class="mb-3">
      ...
      <template #description>
        <span class="text-xs">
          工作台按池整表重建坑位，所以任何其他写入路径都会被覆盖；而概率一旦没闭环，该奖池的抽奖请求会直接报错 ——
          本页刻意不提供编辑入口。
        </span>
      </template>
    </a-alert>
    ...
    >
      <div class="pool-head">
        <div class="pool-title">
          ...
          <a-tag v-if="pool.poolStatus != null" :color="poolStatusOf(pool.poolStatus).color">
            奖池{{ poolStatusOf(pool.poolStatus).desc }}
          </a-tag>
        ...
        <a-button type="link" size="small" :disabled="!pool.activityCode" @click="gotoWorkbench(pool)">
          去工作台修改 →
        </a-button>
      </div>
      ...
      <div class="pool-metrics">
        ...
        <div class="metric">
          ...
          <div class="metric-value" :class="pool.probabilityClosed ? '' : 'metric-broken'">
            {{ num(pool.probabilitySum) }}%
          </div>
        </div>
        <div class="metric">
      ...
      <div v-if="pool.slotList.length > 0" class="prob-bar-wrap">
        <div class="prob-bar">
          <a-tooltip v-for="(slot, i) in pool.slotList" :key="slot.id" :title="slotTooltip(slot)">
            ...
            <div
              class="prob-seg"
              :class="{ 'prob-seg-empty': segSoldOut(slot) }"
              :style="{ width: segWidth(slot), background: segColor(i, slot) }"
            >
      ...
      <a-alert
        v-for="(issue, idx) in pool.issueList"
        :key="idx"
        :type="issue.level === 'DANGER' ? 'error' : 'warning'"
        show-icon
        class="mb-2"
      >
        <template #message><span class="text-xs">{{ issue.message }}</span></template>
      </a-alert>
        ...
        <template #bodyCell="{ record, column }">
          ...
          <template v-if="column.dataIndex === 'stock'">
            ...
            <div v-else class="cell-stack">
              ...
              <div class="cell-sub">
                <span class="cell-label">DB</span>{{ record.remainStockDb ?? '—' }}
                / 总 {{ record.totalStock ?? '—' }}
              </div>
            </div>
          </template>
    ...
    <div v-if="list.length > 0" class="smart-query-table-page">
      <a-pagination
        showSizeChanger
        showQuickJumper
        show-less-items
