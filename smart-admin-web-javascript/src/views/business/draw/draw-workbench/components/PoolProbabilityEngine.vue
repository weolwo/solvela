          ...
          <a-menu-item v-for="pool in poolList" :key="pool.poolCode" @click="activePoolKey = pool.poolCode">
            <div class="group flex items-center justify-between gap-2">
              ...
              <span class="flex shrink-0 items-center gap-1">
                ...
                <a-popconfirm
                  title="确认删除该奖池？池内概率配置将一并丢失"
                  :disabled="isOnline || poolList.length <= 1"
                  @confirm="removePool(pool)"
                >
                  <a-button
                    type="text"
                    size="small"
        ...
        <a-table :data-source="activeMappings" :columns="columns" :pagination="false" row-key="prizeCode" size="small" bordered>
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.dataIndex === 'prizeValue'">
              ¥ {{ formatMoney(prizeMap[record.prizeCode]?.prizeValue) }}
            </template>
...
<script setup>
  ...
  const progressPercent = computed(() =>
    probabilityStatus.value === 'over' ? 100 : Math.min(totalProbability.value.toNumber(), 100)
  );
  ...
  const invalidPoolNames = computed(() =>
    poolList.value
      .filter((pool) => pool.poolCode !== activePoolKey.value && poolStatusOf(pool) !== 'ok')
