...
import sa.activity.manager.ActivityConfigManager;
import sa.draw.poolconfig.domain.entity.PrizePoolConfig;
import sa.draw.poolconfig.domain.form.PrizePoolConfigQueryForm;
import sa.draw.poolconfig.domain.vo.PoolConfigIssueVO;
import sa.draw.poolconfig.domain.vo.PrizePoolBoardResultVO;
import sa.draw.poolconfig.domain.vo.PrizePoolBoardVO;
...
import sa.draw.runtime.DrawPeriodResolver;
...
public class PrizePoolBoardService {
    ...
    private PrizePoolBoardVO analyseOne(PrizePoolConfig pool, Map<String, ActivityConfig> activityMap,
        ...
        boolean poolOpen = POOL_STATUS_OPEN.equals(pool.getStatus());
        ...
        boolean closed = !mappings.isEmpty() && sum.subtract(HUNDRED).abs().compareTo(PROBABILITY_EPSILON) <= 0;
