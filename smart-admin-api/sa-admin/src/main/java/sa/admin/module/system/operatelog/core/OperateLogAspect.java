package sa.admin.module.system.operatelog.core;
...
import sa.admin.module.system.operatelog.OperateLogDao;
import sa.admin.module.system.operatelog.domain.OperateLogEntity;
...
/**
 * 后台操作日志：切 {@code sa.admin} 下所有 Controller 的<b>写操作</b>，落 t_operate_log。
 *
 * <h3>🔴 2026-08-25：切点从「认注解」改成「认包路径」</h3>
 * 原先的切点是
 * {@code @within(...OperateLog) || @annotation(...OperateLog)} —— 只切标了
 * {@code @OperateLog} 的类或方法。问题是<b>全项目只有 SmartAdmin 自带的那几个演示
...
public abstract class OperateLogAspect {
    ...
    /**
     * 只切管理端。C 端（solarx-app）不装配本切面，它那边的行为审计是另一套。
     */
    private static final String POINT_CUT = "execution(* sa.admin..*Controller.*(..))";
