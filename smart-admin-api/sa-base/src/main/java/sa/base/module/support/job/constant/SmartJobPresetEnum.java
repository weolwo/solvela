...
@Getter
@AllArgsConstructor
public enum SmartJobPresetEnum implements BaseEnum {

    /**
     * 轻量高频：探活、状态流转。跑在快车道，错过一次无所谓
     */
    LIGHT("LIGHT", "轻量高频", SmartJobLaneEnum.FAST,
            30, SmartJobBlockStrategyEnum.DISCARD,
            SmartJobMisfireStrategyEnum.SKIP, 60, 30, 0),
     ...
     */
    NORMAL("NORMAL", "常规", SmartJobLaneEnum.SLOW,
            300, SmartJobBlockStrategyEnum.DISCARD,
            SmartJobMisfireStrategyEnum.SKIP, 300, 60, 1),

    /**
     * 重批处理：统计、对账。串行 + 补跑 —— 那天的数据不能少
     */
    HEAVY("HEAVY", "重批处理", SmartJobLaneEnum.SLOW,
            1800, SmartJobBlockStrategyEnum.SERIAL,
            SmartJobMisfireStrategyEnum.FIRE_ONCE, 1800, 0, 2),
     ...
     */
    private final SmartJobLaneEnum lane;

    private final int timeoutSeconds;

    private final SmartJobBlockStrategyEnum blockStrategy;

    private final SmartJobMisfireStrategyEnum misfireStrategy;
     ...
     */
    public boolean matchLane(SmartJobLaneEnum targetLane) {
        return isCustom() || this.lane == targetLane;
    }

    public static SmartJobPresetEnum resolve(String value) {
        for (SmartJobPresetEnum e : values()) {
