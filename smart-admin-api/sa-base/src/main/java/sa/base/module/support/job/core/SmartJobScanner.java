...
import sa.base.module.support.job.constant.SmartJobBlockStrategyEnum;
import sa.base.module.support.job.constant.SmartJobExecuteStatusEnum;
import sa.base.module.support.job.constant.SmartJobMisfireStrategyEnum;
import sa.base.module.support.job.constant.SmartJobTriggerSourceEnum;
import sa.base.module.support.job.constant.SmartJobTriggerTypeEnum;
import sa.base.module.support.job.constant.SmartJobUtil;
import sa.base.module.support.job.repository.SmartJobRepository;
 ...
 * <ol>
 *   <li>扫 {@code t_smart_job}，抢 {@code trigger_version}（定时调度）；</li>
 *   <li>扫 {@code t_smart_job_log}，抢日志行的 {@code status}（手动触发 + 失败重试）。</li>
 * </ol>
...
public class SmartJobScanner {
    ...
    private final SmartJobRunner jobRunner;
    ...
    public SmartJobScanner(SmartJobConfig jobConfig,
        ...
        this.nodeIp = SmartIpUtil.getLocalFirstIp();

        ThreadFactory factory = Thread.ofPlatform().name("smart-job-scanner-", 0).factory();
        ...
        log.info("==== SmartJob ==== 扫描器启动 env={} 间隔={}s 批量={}",
    ...
    private void scanSafely() {
        try {
            ...
            log.error("==== SmartJob ==== 扫描异常（本轮跳过，下轮继续）", t);
        }
    }
    ...
    private void scan() {
        ...
        if (null == dbNow) {
            log.error("==== SmartJob ==== 取数据库时钟失败，本轮跳过");
            return;
        }
    ...
    private void scanScheduledJob(LocalDateTime dbNow) {
        ...
        List<SmartJobEntity> dueList = jobRepository.getJobDao()
                .selectDueJobList(jobConfig.getEnv(), dbNow, jobConfig.getScanBatchSize());
        for (SmartJobEntity job : dueList) {
            try {
                this.handleDueJob(job, dbNow);
            } catch (Throwable t) {
                // 单条任务出错不影响同轮的其它任务
                log.error("==== SmartJob ==== 处理到期任务异常：{}", job.getJobName(), t);
            }
        }
    }

    private void handleDueJob(SmartJobEntity job, LocalDateTime dbNow) {
        Optional<SmartJobHandlerMeta> handlerOpt = handlerRegistry.getHandler(job.getHandlerName());
        if (handlerOpt.isEmpty()) {
            // 标记失联并让它退出调度：不标记的话每秒都会被扫到、每秒报一次错，刷屏还查不出所以然
            log.error("==== SmartJob ==== 🔴 handler 在代码中不存在，任务已停止调度：job={} handler={}",
                    job.getJobName(), job.getHandlerName());
            jobRepository.getJobDao().updateHandlerMissing(job.getJobId(), true);
            return;
        }
        SmartJobHandlerMeta handler = handlerOpt.get();

        // ① 节点级判据：本车道满了就跳过，不抢占。
        //    任务原封不动留在库里，下一轮任何一个有空位的节点自然会接走 ——
        //    这样「本该被别人执行的任务被就地误杀」根本不会发生
        if (!executePool.hasCapacity(handler.lane())) {
            log.debug("==== SmartJob ==== {} 车道已满，跳过（保留待抢）：{}",
        ...
        SmartJobMisfireStrategyEnum misfireStrategy = SmartJobMisfireStrategyEnum.resolve(job.getMisfireStrategy());
        ...
        if (misfired && misfireStrategy == SmartJobMisfireStrategyEnum.SKIP) {
            log.warn("==== SmartJob ==== 错过调度已跳过：job={} 原定={} 现在={}",
                    job.getJobName(), triggerTime, dbNow);
            this.saveTerminalLog(job, triggerTime, dbNow, SmartJobExecuteStatusEnum.MISFIRE,
                    "错过调度窗口，按 SKIP 策略跳过。原定触发 " + triggerTime);
            return;
        }

        this.dispatch(job, handler, triggerTime, dbNow, SmartJobTriggerSourceEnum.SCHEDULE, 0,
     ...
     */
    private boolean isMisfired(SmartJobEntity job, LocalDateTime triggerTime, LocalDateTime dbNow) {
        if (null == triggerTime) {
            return false;
        }
     ...
     * 抢占执行权。影响行数 = 1 才算抢到。
     */
    private boolean preempt(SmartJobEntity job, LocalDateTime dbNow) {
        ...
        if (SmartJobTriggerTypeEnum.ONE_TIME.equalsValue(job.getTriggerType())) {
     ...
     */
    private LocalDateTime calcNextTriggerTime(SmartJobEntity job, LocalDateTime dbNow) {
        if (SmartJobTriggerTypeEnum.ONE_TIME.equalsValue(job.getTriggerType())) {
            // 一次性任务没有下次；置 NULL 由 markTerminal 完成，这里给个远期值只是占位，
            // 实际会被 markTerminal 覆盖成 NULL
            return null;
        }
        LocalDateTime next = SmartJobUtil.nextTriggerTime(job.getTriggerType(), job.getTriggerValue(), dbNow);
        if (null == next) {
            log.error("==== SmartJob ==== 触发配置无法解析，任务将停止调度：job={} value={}",
                    job.getJobName(), job.getTriggerValue());
            return null;
        }
        int jitter = null == job.getJitterSeconds() ? 0 : job.getJitterSeconds();
        return SmartJobUtil.applyJitter(next, job.getJobId(), jitter);
    }
    ...
    private void scanPendingLog(LocalDateTime dbNow) {
        List<SmartJobLogEntity> pendingList = jobRepository.getJobLogDao()
                .selectPendingList(jobConfig.getEnv(), dbNow, jobConfig.getScanBatchSize());
        for (SmartJobLogEntity pending : pendingList) {
            try {
                this.handlePendingLog(pending, dbNow);
            } catch (Throwable t) {
                log.error("==== SmartJob ==== 处理待执行记录异常：logId={}", pending.getLogId(), t);
            }
        }
    }

    private void handlePendingLog(SmartJobLogEntity pending, LocalDateTime dbNow) {
        SmartJobEntity job = jobRepository.getJobDao().selectById(pending.getJobId());
        if (null == job) {
            log.warn("==== SmartJob ==== 待执行记录对应的任务已不存在：logId={}", pending.getLogId());
        ...
        if (this.isBlocked(job, handler, dbNow, pending.getLogId())) {
            ...
            update.setStatus(SmartJobExecuteStatusEnum.BLOCKED.getValue());
        ...
        pending.setStatus(SmartJobExecuteStatusEnum.RUNNING.getValue());
     ...
     */
    private void abandonPending(SmartJobLogEntity pending, String reason) {
        SmartJobLogEntity update = new SmartJobLogEntity();
        update.setLogId(pending.getLogId());
        update.setStatus(SmartJobExecuteStatusEnum.BLOCKED.getValue());
        ...
        jobRepository.getJobLogDao().updateById(update);
        log.warn("==== SmartJob ==== 待执行记录被放弃：logId={} 原因={}", pending.getLogId(), reason);
    }
     ...
     */
    private boolean isBlocked(SmartJobEntity job, SmartJobHandlerMeta handler, LocalDateTime dbNow,
                              Long excludeLogId) {
        SmartJobBlockStrategyEnum strategy = SmartJobBlockStrategyEnum.resolve(job.getBlockStrategy());
        if (strategy == SmartJobBlockStrategyEnum.OVERRIDE) {
            // 中断上一次由超时哨兵负责，这里直接放行
            return false;
        }
     ...
     * 插 RUNNING 日志并投递。
     */
    private void dispatch(SmartJobEntity job, SmartJobHandlerMeta handler,
                          LocalDateTime triggerTime, LocalDateTime dbNow,
                          SmartJobTriggerSourceEnum source, int retrySeq,
        ...
        if (this.isBlocked(job, handler, dbNow, null)) {
            this.saveTerminalLog(job, triggerTime, dbNow, SmartJobExecuteStatusEnum.BLOCKED,
                    "上一次执行尚未结束，按阻塞策略丢弃本次");
            return;
        }
        ...
        logEntity.setStatus(SmartJobExecuteStatusEnum.RUNNING.getValue());
        ...
        try {
            ...
            log.info("==== SmartJob ==== 该触发点已被其它节点执行，本次放弃：job={} trigger={}",
     ...
     */
    private void saveTerminalLog(SmartJobEntity job, LocalDateTime triggerTime, LocalDateTime dbNow,
                                 SmartJobExecuteStatusEnum status, String summary) {
        SmartJobLogEntity logEntity = new SmartJobLogEntity();
        logEntity.setJobId(job.getJobId());
        logEntity.setJobName(job.getJobName());
        logEntity.setAppEnv(job.getAppEnv());
        logEntity.setTraceId(newTraceId());
        logEntity.setTriggerSource(SmartJobTriggerSourceEnum.SCHEDULE.getValue());
        logEntity.setTriggerTime(triggerTime);
        ...
        try {
            ...
            log.debug("==== SmartJob ==== 该触发点记录已存在，跳过：job={}", job.getJobName());
        }
    }
    ...
    private void fillNodeInfo(SmartJobLogEntity logEntity) {
    ...
    public void destroy() {
        ...
        log.info("==== SmartJob ==== 扫描器已停止（不再抢占新任务）");
    }
}
