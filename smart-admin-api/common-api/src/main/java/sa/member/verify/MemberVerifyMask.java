...
public final class MemberVerifyMask {
    ...
    public static String maskName(String name) {
        if (SmartStringUtil.isEmpty(name)) {
            return name;
        }
        ...
        if (length == 2) {
            return SmartStringUtil.hide(name, 1, 2);
        }
        return SmartStringUtil.hide(name, 1, length - 1);
    }
    ...
    public static String maskIdCard(String idCard) {
        if (SmartStringUtil.isEmpty(idCard)) {
            return idCard;
        }
        int length = idCard.codePointCount(0, idCard.length());
        // 长度不对的（脏数据、非大陆证件）一律整体打码，不去猜它的结构
        if (length < 11) {
            return SmartStringUtil.hide(idCard, 0, length);
        }
        return SmartStringUtil.hide(idCard, 6, length - 4);
    }
}
