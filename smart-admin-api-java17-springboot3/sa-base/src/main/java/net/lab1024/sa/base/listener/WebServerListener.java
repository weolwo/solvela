...
public class WebServerListener implements ApplicationListener<WebServerInitializedEvent> {
    ...
    private void showProjectMessage(WebServerInitializedEvent webServerInitializedEvent) {
        ...
        String apiDocsUrl = URLUtil.normalize(String.format("http://localhost:%d%s/v3/api-docs", port, contextPath), false, true);
        log.warn("\n{}\n" +
                        "\t当前启动环境:\t{} , {}" +
                        "\n\t返回码初始化:\t完成{}个返回码初始化" +
                        "\n\t服务本机地址:\t{}" +
                        ...
                        "\n\tOpenAPI文档:\t{}" +
                        "\n-------------------------------------------------------------------------------------\n",
                title, profile, environmentEnum.getDesc(), codeCount, localhostUrl, externalUrl, swaggerUrl, apiDocsUrl);
    }
