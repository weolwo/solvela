 ...
 */
@Slf4j
public class SmartJobLaneExecutePool {

    private final Map<SmartJobLaneEnum, ThreadPoolTaskExecutor> poolMap = new EnumMap<>(SmartJobLaneEnum.class);
    ...
    public SmartJobLaneExecutePool(SmartJobConfig config, SmartJobRunningRegistry runningRegistry) {
        this.shutdownAwaitSeconds = config.getShutdownAwaitSeconds();
        this.runningRegistry = runningRegistry;

        poolMap.put(SmartJobLaneEnum.FAST, buildPool("smart-job-fast-",
                config.getFastCoreSize(), config.getFastMaxSize(), config.getFastQueueCapacity()));
        poolMap.put(SmartJobLaneEnum.SLOW, buildPool("smart-job-slow-",
                config.getSlowCoreSize(), config.getSlowMaxSize(), config.getSlowQueueCapacity()));

        ThreadFactory timeoutFactory = Thread.ofPlatform().name("smart-job-timeout-", 0).factory();
        this.timeoutScheduler = Executors.newSingleThreadScheduledExecutor(timeoutFactory);

        log.info("==== SmartJob ==== 双车道执行池就绪 FAST[{}/{}/{}] SLOW[{}/{}/{}] 停机等待={}s",
                config.getFastCoreSize(), config.getFastMaxSize(), config.getFastQueueCapacity(),
                config.getSlowCoreSize(), config.getSlowMaxSize(), config.getSlowQueueCapacity(),
    ...
    private ThreadPoolTaskExecutor buildPool(String namePrefix, int core, int max, int queue) {
        ...
        pool.setTaskDecorator(new SmartJobMdcTaskDecorator());
     ...
     */
    public boolean hasCapacity(SmartJobLaneEnum lane) {
     ...
     */
    public boolean submit(SmartJobLaneEnum lane, String jobName, Long logId,
                          int timeoutSeconds, Consumer<AtomicBoolean> body) {
        ThreadPoolTaskExecutor pool = poolMap.get(lane);
        if (null == pool) {
            log.error("==== SmartJob ==== 未知车道 {}，任务未投递：{}", lane, jobName);
            return false;
        }
        ...
        try {
            ...
            log.error("==== SmartJob ==== {} 车道已满，任务被拒：{}", lane.getValue(), jobName);
            return false;
        }
        if (timeoutSeconds > 0) {
            ...
            timeoutScheduler.schedule(() -> {
                if (!future.isDone()) {
                    timedOut.set(true);
                    future.cancel(true);
                    log.error("==== SmartJob ==== 任务执行超时({}s)，已发出中断：{}", timeoutSeconds, jobName);
                }
    ...
    @PreDestroy
    public void destroy() {
        log.info("==== SmartJob ==== 执行池开始停机，最长等待 {}s", shutdownAwaitSeconds);
        timeoutScheduler.shutdownNow();
        poolMap.values().forEach(ThreadPoolTaskExecutor::shutdown);
        log.info("==== SmartJob ==== 执行池已停机");
    }
}
