...
| 破损文件兼容性（WPS / Numbers 产物） | POI 真正的护城河，fastexcel 这块基本没保障 → 靠 §13 测试矩阵补 |
...
| **兼容性** | **WPS 生成的 xlsx**、Numbers 导出的 xlsx、`.xls` 改名成 `.xlsx`、加密文件、空文件、非 Excel 文件 |
...
**兼容性那一行是重点**：POI 的价值有很大一部分是"什么破文件都能读"，
换成 fastexcel 后这块保护没了，只能靠测试用例补。
...
| **破损文件兼容性** | POI 真正的护城河。WPS / Numbers 的怪产物只能靠 §13 一个个补 |
