...
> 面向接手人 · 2026-08-03
> 覆盖范围：`solvela-marketing` 的 `solvela.task.runtime` 包（事件入口 → 策略 → 推进 → 发奖）
>
...
`TaskProgressStrategy`（[strategy/TaskProgressStrategy.java:25](../solvela-api/solvela-marketing/src/main/java/sa/task/runtime/strategy/TaskProgressStrategy.java#L25)）
...
两个并发事件各读到 3，各写回 4，用户点了两次只涨了一次。 现在这两条 SQL
各自只有一份（[TaskRecordDao.java:84/95](../solvela-api/solvela-marketing/src/main/java/sa/task/record/dao/TaskRecordDao.java#L84)），
想写错都没地方写。

**② 策略可以纯函数单测。**
没有数据库、没有 Spring 上下文，`plan()` 输入输出都是值对象。断档清零那个经典的 off-by-one （[StreakTaskStrategy.java:62](../solvela-api/solvela-marketing/src/main/java/sa/task/runtime/strategy/StreakTaskStrategy.java#L62)：
...
> （[TaskRecordAdvanceService.java:154](../solvela-api/solvela-marketing/src/main/java/sa/task/runtime/TaskRecordAdvanceService.java#L154)），
...
消费端（[TaskRecordAdvanceService.java:140](../solvela-api/solvela-marketing/src/main/java/sa/task/runtime/TaskRecordAdvanceService.java#L140)）
...
| **必填项被写成"可选"。** `Skip` 若把 `code` 设计成可空，一定会有实现只写 `reason` 图省事，最后大屏上多出一堆归不了类的丢弃                                              | `code` 是 record 的必填分量，**忘了分类就编译不过**（[MetricPlan.java:56](../solvela-api/solvela-marketing/src/main/java/sa/task/runtime/domain/MetricPlan.java#L56) 的注释就是这个意思） |
...
全在 [TaskRecordAdvanceService.java:82-196](../solvela-api/solvela-marketing/src/main/java/sa/task/runtime/TaskRecordAdvanceService.java#L82)。
...
（[:248](../solvela-api/solvela-marketing/src/main/java/sa/task/runtime/TaskRecordAdvanceService.java#L248)）。
...
**🔴 幂等键是
`recordId:stageLevel`**（[TaskConst.java:232](../solvela-api/solvela-marketing/src/main/java/sa/task/constant/TaskConst.java#L232)）。
...
（这段推理原样写在 [TaskConcurrentModifyException.java:14-22](../solvela-api/solvela-marketing/src/main/java/sa/task/runtime/TaskConcurrentModifyException.java#L14)。）
