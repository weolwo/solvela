...
- [任务事件打点-实现技术方案.md](docs/任务事件打点-实现技术方案.md) · 把注册/下单/充值/签到接进任务引擎，而任务系统不长进任何一个业务里
...
**会员与触达**
- [会员等级-实现技术方案.md](docs/会员等级-实现技术方案.md) · 考核周期制，成长值独立于积分（阶段 1-4 全部实现：即时升级 / 期末结算与降级 / 保级缓冲与双倍加速 / 等级专享任务 / C 端等级页；6 表模型，命名统一为 grade）
- [通知与公告-实现技术方案.md](docs/通知与公告-实现技术方案.md) · 消息中心
- [优惠券使用闭环-实现技术方案.md](docs/优惠券使用闭环-实现技术方案.md) · 券目前只能发不能用（尚未实现）
