 ...
 */
@Data
public class ProposalFunnelDTO {
    ...
    /** 部分成功：status=60，发了一半，需要人工补齐 */
    private Long partialCount;

    /** 彻底失败：status=70 */
    private Long failedCount;

    /** 风控拦截：status=80，钱没出去，是防线生效 */
    ...
    /** 资产维度发放情况，金额按资产类型分开算 */
    private List<AssetStatDTO> assetList;

    /** 来源维度分布，按提案数降序 */
    private List<SourceStatDTO> sourceList;

    /** 风控拦截原因分布（TOP 10） */
    private List<BlockReasonDTO> blockReasonList;
     ...
     */
    @Data
    public static class AssetStatDTO {
        ...
        /** 已发出金额/数量 = SUM(amount × quantity)，仅统计成功的 */
        private BigDecimal successAmount;

        /** 在途金额：待审 + 待执行 + 执行中，钱还没出去但已被占用 */
     ...
     */
    @Data
    public static class BlockReasonDTO {

        /** 拦截分类编码，对齐 RiskBlockCode；历史数据可能为 null */
        private String riskCode;

        /** 拦截原因：能归类的取分类说明，归不了类的回显 remark 原文 */
