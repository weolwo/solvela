 ...
 *
 * <p>判据是 {@link SolvelaJobLogContext#current()} 不为空 —— 也就是说
 ...
 * @date 2026-08-11
 */
public class SolvelaJobLogAppender extends AppenderBase<ILoggingEvent> {

    private static final DateTimeFormatter TIME_FORMAT =
            DateTimeFormatter.ofPattern("HH:mm:ss.SSS").withZone(ZoneId.systemDefault());

    private final SolvelaJobLogCollector collector;
    ...
    public SolvelaJobLogAppender(SolvelaJobLogCollector collector, String levelName) {
        this.collector = collector;
        // 解析不出来时按 INFO —— 配错一个字母不该导致「全采」这种最坏结果
        this.threshold = Level.toLevel(levelName, Level.INFO);
        this.setName("solvelaJobLogAppender");
    }

    @Override
    protected void append(ILoggingEvent event) {
        Long logId = SolvelaJobLogContext.current();
        if (null == logId) {
