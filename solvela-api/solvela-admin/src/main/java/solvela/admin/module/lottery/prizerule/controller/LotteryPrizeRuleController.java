...
public class LotteryPrizeRuleController {
    ...
    @RequiresPermission("lotteryPrizeRule:query")
    public PageResult<LotteryPrizeRuleVO> queryPage(@RequestBody @Valid LotteryPrizeRuleQueryForm queryForm) {
        PageResult<LotteryPrizeRuleDTO> page = Service.queryPage(SolvelaBeanUtil.copy(queryForm, LotteryPrizeRuleQuery.class));
        return SolvelaPageUtil.convert2PageResult(page, LotteryPrizeRuleVO.class);
    }

    @Operation(summary = "奖励结构分析：按玩法给出净中奖率、预计中奖注数、预计赔付成本与配置体检告警")
    @PostMapping("/analysis")
    @RequiresPermission("lotteryPrizeRule:query")
    public LotteryPrizeAnalysisResultDTO analysis(@RequestBody @Valid LotteryPrizeRuleQueryForm queryForm) {
        return lotteryPrizeAnalysisService.analysis(SolvelaBeanUtil.copy(queryForm, LotteryPrizeRuleQuery.class));
    }
}
