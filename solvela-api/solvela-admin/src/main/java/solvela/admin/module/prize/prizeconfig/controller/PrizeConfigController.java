...
public class PrizeConfigController {
    ...
    public PageResult<PrizeConfigVO> queryPage(@RequestBody @Valid PrizeConfigQueryForm queryForm) {
        PageResult<PrizeConfigDTO> page = Service.queryPage(SolvelaBeanUtil.copy(queryForm, PrizeConfigQuery.class));
        return SolvelaPageUtil.convert2PageResult(page, PrizeConfigVO.class);
    }

    @Operation(summary = "查询活动下启用中的奖品（抽奖工作台资产大库抽屉用）")
    @GetMapping("/optionList")
    @RequiresPermission("prizeConfig:query")
    public List<PrizeConfigVO> queryEnabledList(@RequestParam String activityCode) {
        return SolvelaBeanUtil.copyList(Service.queryEnabledList(activityCode), PrizeConfigVO.class);
    }
    ...
    @PostMapping("/add")
    @RequiresPermission("prizeConfig:add")
    public void add(@RequestBody @Valid PrizeConfigAddForm addForm) {
    ...
    public void updateStatus(@RequestBody @Valid PrizeStatusUpdateForm form) {
    ...
    @PostMapping("/update")
    @RequiresPermission("prizeConfig:update")
    public void update(@RequestBody @Valid PrizeConfigUpdateForm updateForm) {
