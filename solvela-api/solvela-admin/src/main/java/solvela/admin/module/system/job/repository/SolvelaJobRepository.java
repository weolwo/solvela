...
@Slf4j
@Service
public class SolvelaJobRepository {
    ...
    public SolvelaJobDao getJobDao() {
        return jobDao;
    }

    public SolvelaJobLogDao getJobLogDao() {
        return jobLogDao;
    }
    ...
    public void saveLog(SolvelaJobLogEntity logEntity) {
        jobLogDao.insert(logEntity);
        SolvelaJobEntity updateJob = new SolvelaJobEntity();
     ...
     */
    @Transactional(rollbackFor = Throwable.class)
    public void saveLogOnly(SolvelaJobLogEntity logEntity) {
        jobLogDao.insert(logEntity);
        SolvelaJobEntity updateJob = new SolvelaJobEntity();
        updateJob.setJobId(logEntity.getJobId());
        updateJob.setLastExecuteLogId(logEntity.getLogId());
     ...
     */
    @Transactional(rollbackFor = Throwable.class)
    public void finishAndScheduleRetry(SolvelaJobLogEntity finishedLog,
                                       SolvelaJobLogEntity retryEntity,
                                       Integer jobId,
                                       int failIncrement,
                                       Long finishedLogId) {
