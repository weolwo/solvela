 ...
 * 所以配了四道闸门，缺一不可：
 * <ol>
 *   <li><b>目标白名单</b>（{@code solvela.job.http-allow-hosts}）配在 yaml 里，
...
public class SolvelaJobHttpCallerHandler implements SolvelaJob {
    ...
    public SolvelaJobHttpCallerHandler(SolvelaJobConfig jobConfig) {
    ...
    @Override
    public String execute(SolvelaJobContext ctx) throws Exception {
    ...
    private void checkGate(URI uri) {
        ...
        if (null == allowHosts || allowHosts.isEmpty()) {
            ...
            throw new IllegalStateException(
                    "未配置 solvela.job.http-allow-hosts 白名单，HTTP 任务默认不可用");
        }
