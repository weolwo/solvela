...
/**
 * 把 {@link SolvelaJobLogAppender} 挂到 root logger 上。
 *
 ...
 */
@Slf4j
public class SolvelaJobLogAppenderRegistrar {

    private SolvelaJobLogAppender appender;

    public SolvelaJobLogAppenderRegistrar(SolvelaJobLogCollector collector, String levelName) {
        ILoggerFactory factory = LoggerFactory.getILoggerFactory();
        if (!(factory instanceof LoggerContext loggerContext)) {
            log.warn("==== SolvelaJob ==== 当前日志实现不是 Logback，实时执行日志功能不可用");
            return;
        }
        this.appender = new SolvelaJobLogAppender(collector, levelName);
        this.appender.setContext(loggerContext);
        this.appender.start();

        Logger rootLogger = loggerContext.getLogger(Logger.ROOT_LOGGER_NAME);
        rootLogger.addAppender(appender);
        log.info("==== SolvelaJob ==== 执行日志采集器已挂载（仅采集任务线程，级别 >= {}）", levelName);
    }
