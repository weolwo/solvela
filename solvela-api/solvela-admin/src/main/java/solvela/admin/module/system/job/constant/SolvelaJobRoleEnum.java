 ...
 * 原实现里管理接口 {@code SolvelaJobService} 与消息通道都挂着
 * {@code @ConditionalOnBean(SolvelaJobAutoConfiguration.class)}，
 * 而那个配置类由 {@code solvela.job.enabled} 控制 ——
 * 于是「业务节点不执行任务、但仍能在后台管理任务」这个诉求<b>物理上做不到</b>：
 * 关掉 enabled，后台菜单直接报错。
 *
 * <p>而这恰恰是独立部署要的形态：同一份 fat jar，靠启动参数分角色。
 * <pre>
 * java -jar solvela-admin.jar --solvela.job.role=ADMIN    # 业务节点：能管不能跑
 * java -jar solvela-admin.jar --solvela.job.role=WORKER   # 任务节点：能跑不接流量
 * </pre>
...
@Getter
@AllArgsConstructor
public enum SolvelaJobRoleEnum implements BaseEnum {
