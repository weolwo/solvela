...
public class AdminSolvelaJobController extends SupportBaseController {
    ...
    @RepeatSubmit
    public void execute(@RequestBody @Valid SolvelaJobExecuteForm executeForm) {
        RequestEmployee requestUser = CurrentEmployee.orNull();
        executeForm.setUpdateName(requestUser.getUserName());
        jobService.execute(executeForm);
    }

    @Operation(summary = "定时任务-查询详情 @huke")
    @GetMapping("/job/{jobId}")
    public SolvelaJobVO queryJobInfo(@PathVariable Integer jobId) {
        return jobService.queryJobInfo(jobId);
    }

    @Operation(summary = "定时任务-分页查询 @huke")
    @PostMapping("/job/query")
    public PageResult<SolvelaJobVO> queryJob(@RequestBody @Valid SolvelaJobQueryForm queryForm) {
    ...
    @PostMapping("/job/add")
    @RepeatSubmit
    public void addJob(@RequestBody @Valid SolvelaJobAddForm addForm) {
        RequestEmployee requestUser = CurrentEmployee.orNull();
        addForm.setUpdateName(requestUser.getUserName());
        jobService.addJob(addForm);
    }

    @Operation(summary = "定时任务-更新-任务信息 @huke")
    @PostMapping("/job/update")
    @RepeatSubmit
    public void updateJob(@RequestBody @Valid SolvelaJobUpdateForm updateForm) {
        RequestEmployee requestUser = CurrentEmployee.orNull();
        updateForm.setUpdateName(requestUser.getUserName());
        jobService.updateJob(updateForm);
    }

    @Operation(summary = "定时任务-更新-开启状态 @huke")
    @PostMapping("/job/update/enabled")
    @RepeatSubmit
    public void updateJobEnabled(@RequestBody @Valid SolvelaJobEnabledUpdateForm updateForm) {
        RequestEmployee requestUser = CurrentEmployee.orNull();
        updateForm.setUpdateName(requestUser.getUserName());
        jobService.updateJobEnabled(updateForm);
    }

    @Operation(summary = "定时任务-删除 @zhuoda")
    @GetMapping("/job/delete")
    @RepeatSubmit
    public void deleteJob(@RequestParam Integer jobId) {
        jobService.deleteJob(jobId, CurrentEmployee.orNull());
    }

    @Operation(summary = "定时任务-执行记录-分页查询 @huke")
    @PostMapping("/job/log/query")
    public PageResult<SolvelaJobLogVO> queryJobLog(@RequestBody @Valid SolvelaJobLogQueryForm queryForm) {
    ...
    @RepeatSubmit
    public void rerun(@RequestParam Long logId) {
        jobService.rerun(logId, CurrentEmployee.nameOrNull());
    }
    ...
    @RepeatSubmit
    public String terminate(@RequestParam Long logId) {
    ...
    public SolvelaJobTriggerPreviewVO previewTriggerTime(
    ...
    public List<String> queryExecuteLog(@RequestParam Long logId) {
