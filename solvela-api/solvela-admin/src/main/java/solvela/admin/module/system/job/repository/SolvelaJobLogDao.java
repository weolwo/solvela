 ...
 */
@Mapper
public interface SolvelaJobLogDao extends BaseMapper<SolvelaJobLogEntity> {

    /**
     * 定时任务-执行记录-分页查询
     */
    List<SolvelaJobLogVO> query(Page<?> page, @Param("query") SolvelaJobLogQueryForm queryForm);
     ...
     */
    List<SolvelaJobLogEntity> selectPendingList(@Param("appEnv") String appEnv,
                                                @Param("dbNow") LocalDateTime dbNow,
                                                @Param("limit") int limit);

    /**
     * 🔴 抢占日志行：抢的是 {@code t_solvela_job_log.status}，<b>不是</b> {@code job.trigger_version}。
     *
     ...
     */
    List<SolvelaJobLogEntity> selectZombieList(@Param("beforeTime") LocalDateTime beforeTime,
                                               @Param("limit") int limit);
