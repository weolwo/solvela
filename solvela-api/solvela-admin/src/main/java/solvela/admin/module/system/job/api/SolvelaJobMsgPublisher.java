 ...
 * 只负责「往执行端发消息」的一半。
 *
 * <p>🔴 <b>从 {@link SolvelaJobClientManager} 里拆出来，是为了让 ADMIN 节点能用。</b>
 * 原实现把「发消息」和「收消息并执行」焊在同一个类里，而那个类依赖执行侧的组件 ——
 * 于是不执行任务的节点连「通知别人去执行」都做不到，
 * 后台的「立即执行」「保存后生效」全都失灵。这正是独立部署被卡住的地方之一。
 *
 * <p>拆开之后：ADMIN 只装本类（发），WORKER 装 {@link SolvelaJobClientManager}（收 + 执行），
 * ALL 两个都装。
 *
 * @author alaric
 * @date 2026-08-11
 */
@Slf4j
public class SolvelaJobMsgPublisher {

    private static final String TOPIC_PREFIX = "solvela-job-instance:";

    private final RTopic topic;

    public SolvelaJobMsgPublisher(SolvelaJobConfig jobConfig, RedissonClient redissonClient) {
        this.topic = redissonClient.getTopic(TOPIC_PREFIX + jobConfig.getEnv());
        log.info("==== SolvelaJob ==== msg-publisher init, topic={}", TOPIC_PREFIX + jobConfig.getEnv());
    }
     ...
     */
    public void publishToClient(SolvelaJobMsg msgDTO) {
        msgDTO.setMsgId(SolvelaRandomUtil.simpleUuid());
