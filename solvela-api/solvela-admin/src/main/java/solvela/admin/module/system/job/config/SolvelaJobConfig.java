...
/**
 * solvela job 配置，与配置文件参数对应
 *
 * @author huke
 * @date 2024/6/17 21:30
 */
@ConfigurationProperties(prefix = SolvelaJobConfig.CONFIG_PREFIX)
@Data
public class SolvelaJobConfig {

    public static final String CONFIG_PREFIX = "solvela.job";
     ...
     */
    private SolvelaJobRoleEnum role = SolvelaJobRoleEnum.ALL;
     ...
     * {@code solvela-job-instance}，跨环境串台没有任何阻拦。
     */
    ...
    //    详见 SolvelaJobScanner#scanScheduledJob 的注释。
    //    留一个不起作用的旋钮比没有更糟 —— 会让人以为调它有用。

    /**
     * 任务默认超时秒数；0 表示不限。
     *
     * <p>执行器可用 {@code @SolvelaJobHandler#defaultTimeoutSeconds()} 单独声明，
     * 声明值优先。
     */
