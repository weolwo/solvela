 ...
 * <p>做两件事：
 * <ol>
 *   <li><b>注册</b>：扫描所有 {@link SolvelaJob} Bean，读 {@link SolvelaJobHandler} 注解建表；</li>
 ...
 */
@Slf4j
public class SolvelaJobHandlerRegistry implements SmartInitializingSingleton {
     ...
     */
    private final Map<String, SolvelaJobHandlerMeta> handlerMap = new LinkedHashMap<>();

    private final ObjectProvider<SolvelaJob> jobBeanProvider;
    private final boolean isNoneRole; // 新增

    // 2. 构造函数直接接收
    public SolvelaJobHandlerRegistry(ObjectProvider<SolvelaJob> jobBeanProvider,
    ...
    private void register() {
        ...
        List<SolvelaJob> jobBeanList = isNoneRole
                ? List.of()
                : jobBeanProvider.orderedStream().toList();
        for (SolvelaJob bean : jobBeanList) {
            Class<?> targetClass = AopUtils.getTargetClass(bean);
            SolvelaJobHandler anno = AnnotationUtils.findAnnotation(targetClass, SolvelaJobHandler.class);
            if (null == anno) {
                // 没打注解的实现类：不是错误，但它永远不可能被调度到，必须说出来
                log.warn("==== SolvelaJob ==== 实现了 SolvelaJob 但未标注 @SolvelaJobHandler，将永远不会被调度：{}",
                        targetClass.getName());
                continue;
            }
            String name = anno.name();
            if (null == name || name.isBlank()) {
                throw new IllegalStateException("@SolvelaJobHandler.name 不能为空：" + targetClass.getName());
            }
            SolvelaJobHandlerMeta exist = handlerMap.get(name);
            if (null != exist) {
                // 🔴 重名直接让启动失败。静默取其一是最坏的结果：
                //    两个执行器抢同一个名字，跑起来的是哪个取决于 Bean 扫描顺序，
                //    换台机器、换个 JDK 版本都可能变 —— 属于最难复现的一类故障。
                throw new IllegalStateException(String.format(
                        "@SolvelaJobHandler 名称重复：name=%s 同时被 %s 和 %s 使用",
                        name, exist.handlerClassName(), targetClass.getName()));
            }
            handlerMap.put(name, new SolvelaJobHandlerMeta(
        ...
        log.info("==== SolvelaJob ==== handler 注册完毕，共 {} 个：{}", handlerMap.size(), handlerMap.keySet());
    }
     ...
     */
    public Optional<SolvelaJobHandlerMeta> getHandler(String handlerName) {
     ...
     */
    public Collection<SolvelaJobHandlerMeta> getAllHandler() {
