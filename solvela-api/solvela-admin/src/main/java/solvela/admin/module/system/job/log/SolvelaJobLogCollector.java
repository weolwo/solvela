 ...
 * 任务执行到一半卡住时，{@code t_solvela_job_log} 里只有<b>结束时</b>才写入的
 ...
 */
@Slf4j
public class SolvelaJobLogCollector {

    private static final String KEY_PREFIX = "solvela-job-log:";
    ...
    public SolvelaJobLogCollector(RedissonClient redissonClient) {
    ...
    public void append(Long logId, String line) {
        ...
        try {
            ...
            // 采集失败绝不能影响任务本身
            log.warn("==== SolvelaJob ==== 执行日志采集失败 logId={}", logId, e);
        }
    }
    ...
    public List<String> read(Long logId) {
        try {
            ...
            log.warn("==== SolvelaJob ==== 执行日志读取失败 logId={}", logId, e);
            return List.of();
        }
    }
}
