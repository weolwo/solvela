...
import solvela.app.config.SharedControllerExcludeFilter;
 ...
 * </ul>
 *
 * <h3>为什么要排除共享模块（solvela.app 之外）的 controller</h3>
 * 本模块依赖 solvela-base 与 common-api 是为了复用领域层（service / manager / dao），
 * 但它们自带的 controller 是<b>管理端视角</b>的接口。如果照 AdminApplication 那样直接
 * {@code @ComponentScan("solvela")} 全量扫，这些 controller 会连同服务一起注册到 1025 端口上。
 * 拦截器确实会拦住未登录请求，但那意味着<b>安全性依赖于「拦截器没写错」这一个假设</b>，
 * 而正确答案是这些路由压根不该存在于这个进程里。
 *
 * <p>判据见 {@link SharedControllerExcludeFilter}：按 {@code @Controller} 注解认，不按包名认 ——
 * 早先用包名正则时漏掉过 {@code ConfigController} / {@code TableColumnController}
 * 这两个没放在 controller 包下的类，实测确实注册上了。
 *
 * <p>🔴 本模块的接口<b>一律写在 {@code solvela.app.module.**.controller} 下</b>，
 * 需要复用某个查询时注入它的 service，不要试图把共享模块的 controller 放进来。
 *
 * <h3>刻意没有 @EnableScheduling</h3>
 * 管理端已经开了。两个进程连的是<b>同一个库同一个 Redis</b>，再开一次等于所有 {@code @Scheduled}
 * 任务每天跑两遍 —— 发奖、结算这类任务重复执行，多发的那一份没人会主动来报。
 * 定时任务归管理端进程所有，本服务只处理请求。同理 {@code solvela.job.enabled} 在
...
@ComponentScan(
        basePackages = AppApplication.COMPONENT_SCAN,
        excludeFilters = @ComponentScan.Filter(
                type = FilterType.CUSTOM,
                classes = SharedControllerExcludeFilter.class))
// Dao 与 controller 相反，必须全量扫：会员、钱包、券的 Mapper 都在共享模块里
@MapperScan(value = AppApplication.COMPONENT_SCAN, annotationClass = Mapper.class)
