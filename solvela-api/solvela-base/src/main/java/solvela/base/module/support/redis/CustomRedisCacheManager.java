...
import static solvela.base.common.constant.StringConst.COLON;
...
public class CustomRedisCacheManager extends RedisCacheManager {
    ...
    protected RedisCache createRedisCache(String name, RedisCacheConfiguration cacheConfig) {
        ...
        CacheKeyPrefix keyPrefix = cacheName -> {
            if (SolvelaStringUtil.isBlank(cacheName)) {
                return CACHE_PREFIX + COLON;
            }
            String[] parts = cacheName.split(CUSTOM_TTL_SEPARATOR, 2);
            String cleanName = SolvelaStringUtil.trim(parts[0]);
    ...
    private Duration parseTtlFromCacheName(String name) {
        if (SolvelaStringUtil.isBlank(name)) {
            return null;
        }
        ...
        String ttlStr = SolvelaStringUtil.trim(parts[1]);
        if (SolvelaStringUtil.isBlank(ttlStr)) {
            return null;
        }
