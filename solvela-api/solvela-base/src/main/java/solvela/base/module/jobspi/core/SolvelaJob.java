 ...
 * 定时任务 执行接口。
 *
 * <p>实现类必须同时标注 {@link SolvelaJobHandler} 并注册为 Spring Bean，
 ...
 * 与库里的配置对不上，任务静默不跑。匹配现在一律走 {@code @SolvelaJobHandler#name()}。
 *
 * <p><b>写执行器时的四条硬约束：</b>
 * <ol>
 *   <li>🔴 <b>响应中断。</b> 超时靠 {@code Future#cancel(true)}，本质是
 *       {@link Thread#interrupt()} —— 不响应中断的任务砍不掉，超时配了也白配。
 *       循环处理批量数据时，每批开头调一次 {@link SolvelaJobContext#checkCancelled()}；
 *       发起 HTTP / 远程调用时，自己带上连接与读取超时。</li>
 *   <li>🔴 <b>时间只用 {@code ctx}，不写 {@code LocalDateTime.now()}</b>（铁律 9/10）。
 *       {@link SolvelaJobContext#dbNow()} / {@link SolvelaJobContext#bizDate()} 都来自数据库时钟。
 ...
 *       {@code @SolvelaJobHandler(idempotent = true)} 的执行器才允许配失败重试。
...
public interface SolvelaJob {
     ...
     *                   抛 {@link SolvelaJobCancelledException} 则落成「超时中断」而非「失败」
     */
    String execute(SolvelaJobContext ctx) throws Exception;
}
