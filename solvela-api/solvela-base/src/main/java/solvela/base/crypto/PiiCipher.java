 ...
 *
 * <h3>密钥</h3>
 * 取自配置 {@code solvela.crypto.pii.secret-key}，经 SHA-256 派生成 32 字节，所以配置串长度随意。
...
public class PiiCipher {
    ...
    public PiiCipher(@Value("${solvela.crypto.pii.secret-key:}") String secretKey) {
        if (secretKey == null || secretKey.isBlank()) {
            // 🔴 不给默认密钥。给了的话，谁忘了配置就会用一个全世界都知道的密钥把用户地址加密进库，
            //    而且一切看起来都正常 —— 这种「配置漏了但功能照跑」正是最难发现的一类问题。
            throw new IllegalStateException(
                    "solvela.crypto.pii.secret-key 未配置：收件人信息是个人信息，必须加密落库，"
