...
/**
 * {@link SolvelaContentDispositionUtil}：RFC 6266 / 8187 一致性，以及 header 截断防护。
 *
 * @Date 2026-08-10
 */
class SolvelaContentDispositionUtilTest {

    @Test
    @DisplayName("双写法：filename= 在前给老 UA，filename*= 在后给现代浏览器")
    void bothForms() {
        String header = SolvelaContentDispositionUtil.attachment("年会.png");
    ...
    void spaceIsPercentTwenty() {
        String header = SolvelaContentDispositionUtil.attachment("my report.pdf");
    ...
    void semicolonAndCommaEncoded() {
        String header = SolvelaContentDispositionUtil.attachment("a;b,c.png");
    ...
    void attrCharsStayLiteral() {
        assertThat(SolvelaContentDispositionUtil.attachment("a-b_c.d~e.png"))
    ...
    void pureChineseKeepsExtension() {
        assertThat(SolvelaContentDispositionUtil.attachment("活动主视觉.jpeg"))
    ...
    void noExtension() {
        assertThat(SolvelaContentDispositionUtil.attachment("报告"))
    ...
    void controlCharsRemoved() {
        String header = SolvelaContentDispositionUtil.attachment("a\r\nSet-Cookie: x=1.png");
    ...
    void inlineVariant() {
        assertThat(SolvelaContentDispositionUtil.inline("年会.png")).startsWith("inline; ");
        assertThat(SolvelaContentDispositionUtil.attachment("年会.png")).startsWith("attachment; ");
    }

    @Test
    @DisplayName("空文件名只返回 type，不拼出半截参数")
    void blankFileName() {
        assertThat(SolvelaContentDispositionUtil.attachment(null)).isEqualTo("attachment");
        assertThat(SolvelaContentDispositionUtil.attachment("  ")).isEqualTo("attachment");
    }
}
