 ...
 * 原先 {@code SolvelaJob#getClassName()} 返回 {@code this.getClass().getName()}，
 ...
 *
 * <p>名字一旦上线就等同于一份契约：库里的 {@code t_solvela_job.handler_name} 指向它。
 * 要改名，得连同数据一起迁。
 *
 * @author alaric
 * @date 2026-08-11
...
public @interface SolvelaJobHandler {

    /**
     * 执行器名称，对应 {@code t_solvela_job.handler_name}。
     *
     * <p>全局唯一。重名会让启动<b>直接失败</b>而不是静默取其一 —— 见
     * {@link SolvelaJobHandlerRegistry}。
     */
    String name();
     ...
     *
     * <p>声明 {@link SolvelaJobLaneEnum#FAST} 就等于承诺「我 30 秒内跑完」，
     * 框架会把超时硬性压到 {@link SolvelaJobLaneEnum#FAST_MAX_TIMEOUT_SECONDS} 秒并按时中断 ——
     * <b>声明必须被执行强制，否则隔离只存在于注释里。</b>
     */
    SolvelaJobLaneEnum lane() default SolvelaJobLaneEnum.SLOW;
