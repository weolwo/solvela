package solvela.member.server.web;
 ...
 *
 * <p>⚠️ 与 {@code solvela.marketing.server.web.InternalExceptionHandler} 是<b>同一份契约</b>
 * （同样的 code、同样的状态码映射）。两个服务各留一份而不是抽公共模块，是因为现在只有两处；
 * <b>出现第三个服务时就该抽出去</b> —— 在那之前，改一处必须改另一处，
 * 否则网关会看到两个下游对同一类错误给出不同的状态码。
 *
