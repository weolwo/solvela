...
public class MqMessageLogService {
    ...
    // ------------------------------------------------------------------
    // 清理：只保留最近 N 天，且只删【成功的】
    // ------------------------------------------------------------------

    /**
     * 统计可清理的行数（试运行用）。
     *
     * <p>🔴 判据是「成功 <b>且</b> 过了保留期」。失败的行不管多久都不删 ——
     * 它们是「这条消息没处理成功」的唯一证据，删掉等于把问题抹平了。
     */
    public long countCleanable(LocalDateTime deadline) {
        return mqMessageLogDao.selectCount(Wrappers.<MqMessageLog>lambdaQuery()
                .eq(MqMessageLog::getStatus, MqMessageStatusEnum.SUCCESS)
                .lt(MqMessageLog::getReceiveTime, deadline));
    }

    /**
     * 删一批。分批不是性能优化 —— 一次删几十万行会长时间持有行锁、撑爆 binlog。
     *
     * @return 实际删除行数；小于 batchSize 表示已经删完
     */
    public int cleanBatch(LocalDateTime deadline, int batchSize) {
        return mqMessageLogDao.delete(Wrappers.<MqMessageLog>lambdaQuery()
                .eq(MqMessageLog::getStatus, MqMessageStatusEnum.SUCCESS)
                .lt(MqMessageLog::getReceiveTime, deadline)
