 ...
 */
/*
 * 三个端共用一份（2026-08-30 从 solvela.app.web 搬到这里）。
 *
 * 🔴 拆成四个进程之后，这个过滤器不只是「给日志加个 id」——
 * 它是【跨服务链路能不能串起来】的另一半：
 *   · 调用方：DownstreamClientConfig 的拦截器把 Trace.id() 写进请求头；
 *   · 被调方：本过滤器把它读回 MDC。
 * 少了任何一半，两个服务的日志就只能靠时间戳对 —— 而并发下时间戳对不出因果。
 *
 * 抄三份的话，三处的 sanitize 规则迟早不一致：网关认的 id 被下游判非法、
 * 于是下游自己生成一个新的，链路照样断，而且断得很隐蔽（两边都有 id，只是不一样）。
 */
@Component
