...
/**
 * 服务间调用的<b>统一</b>错误出口，marketing 与 member 共用一份。
 *
 * <h3>🔴 它刻意不在 solvela.base 包下</h3>
 * 三个端都扫 {@code solvela.base}。本类若放在那底下，<b>网关会连带扫到</b> ——
 * 而网关有自己的 C 端错误契约（{@code ApiErrors} + {@code ApiErrorResponse}）。
 * 两个 {@code @RestControllerAdvice} 同时在场时，返回什么取决于哪个先匹配上，
 * 表现是「同一个异常，客户端有时认得有时不认得」。
 * {@code AppBoundaryTest} 的「本进程只有一个错误出口」断言就是防这件事的。
 *
 * <p>所以它在 {@code solvela.server.internal} 下，由两个内部服务<b>显式多扫一个包</b>。
 * 包根的差别不是风格，是可见性的开关。
 *
