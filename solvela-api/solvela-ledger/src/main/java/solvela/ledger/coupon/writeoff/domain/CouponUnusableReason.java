...
public enum CouponUnusableReason {
    ...
    /**
     * 这一单<b>没有</b>它能抵的那一部分：纯积分单上的现金券、纯现金单上的积分券。
     *
     * <p>🔴 <b>不要</b>为了让它可用而去换算 —— 1 积分 ≠ 1 元，汇率是业务定义、还会变。
     * 系统替用户做这个换算，就是替他做了一个没有依据的决定。
     *
     * <p>⚠️ 2026-09-15 起这一档<b>比以前少见得多</b>：混合支付单（积分 + 现金）
     * 两种券都用得上，各减各的那一半。以前它会在混合单上误伤所有 {@code SCORE} 券。
     */
    DEDUCT_TARGET_MISMATCH("这一单没有可以用它抵扣的部分"),
    ;
